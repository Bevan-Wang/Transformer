...
src_len = 5 # (原句子的长度)enc_input max sequence length
...
def get_attn_pad_mask(seq_q,seq_k):
    ...
    seq_len could be src_len or it could be tgt_lennvdia
...
def get_attn_subsequence_mask(seq):
    ...
    subsequence_mask = np.triu(np.ones(atten_shape),k=1)  # 生成一个上三角矩阵
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask, dec_enc_attn_mask):
        ...
        dec_outputs, dec_enc_attn = self.dec_enc_attn(dec_outputs, enc_outputs, enc_outputs, dec_enc_attn_mask)  #Attention层的Q（来自decoder）和K,V(来自encoder)
        dec_outputs = self.pos_ffn(dec_outputs)  # [batch_size, tgt_len, d_model]
        return dec_outputs, dec_self_attn, dec_enc_attn  #dec_self_attn, dec_enc_attn这两个是为了可视化
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.tgt_emb = nn.Embedding(tgt_vocab_size, d_model)  # Decoder输入的embedding词表
        self.pos_emb = PositionalEncoding(d_model)  # Transformer中位置编码是固定的，不需要学习
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        # Decoder输入序列的pad mask矩阵（这个例子中Decoder是没有加pad的，实际应用中都是由pad填充的）
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        # dec_outputs: [batch_size, tgt_len, d_model], dec_self_attns: [n_layers, batch_size, n_heads, tgt_len, tgt_len], dec_enc_attns: [n_layers, batch_size, tgt_len, src_len]
